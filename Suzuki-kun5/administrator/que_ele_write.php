...
if (preg_match("/^http:\/\/.*\/administrator\/.*\?mode=(que|ele)$/",$_SERVER["HTTP_REFERER"])) {
	...
	if (!isset($_POST['add-e'])) {
		...
		if (isset($_GET['del_ele_id'])) {
			...
			$sql .= "kind varchar(50),";
			$sql .= "who varchar(50),";
			$sql .= "faculty varchar(10),";
			$sql .= "grade varchar(10),";
			$sql .= "subject varchar(10),";
			$sql .= "name varchar(70) character set utf8,";
			$sql .= "comment text character set utf8,";
			$sql .= "edit varchar(10),";
			$sql .= "writer varchar(70),";
			$sql .= "output varchar(10),";
			$sql .= "date datetime,";
			$sql .= "primary key (id)";
			$sql .= ")";
