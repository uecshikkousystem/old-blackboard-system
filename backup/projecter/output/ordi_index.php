...
if (!empty($row_now['faculty']) && !empty($row_now['grade']) && !empty($row_now['subject']) && !empty($row_now['fname']) && !empty($row_now['lname'])) {
	...
	echo "<div class=\"title-name-1\">議案提出者</div><div class=\"title-name-2\">" . $row_now['grade'] . $row_now['subject'] . "</div><div class=\"title-name-3\">" . parts_change(f_parts_change($row_now['fname'])) . "<div class=\"name-space\"><hr /></div>" . parts_change(f_parts_change($row_now['lname'])) . "</div></div>\n";
...
while ($row = mysql_fetch_array($res,MYSQL_ASSOC)) {
	...
	if (preg_match("/^motion$/",$row["kind"])) {
		...
		if (!preg_match("/^general$/",$row["who"])) {
			...
			eif ($row['grade']=='1'or$row['grade']=='2') {
				if ($row['subject']=='その他'){
