  <div class="main">
    <form class="form" method="post" name="form" action="que_ele_write.php">
      <div class="faculty">
        <fieldset>
          <legend>学部・学域</legend>
      ...
      <div class="grade">
        <fieldset>
          ...
          <div class="grade-in" id='grade-in'>
			<input type="radio" name="grade" id="grade-1" value="1" <?php if (!isset($_GET['re'])) { if (preg_match("/^1$/",$grade)){ echo "checked=\"checked\""; }} else { if (empty($_SESSION['error4']) and preg_match("/^1$/",$_SESSION['grade'])){ echo "checked=\"checke    d\""; }} ?> /><label for="grade-1">1</label>
                      <input type="radio" name="grade" id="grade-2" value="2" <?php if (!isset($_GET['re'])) { if (preg_match("/^2$/",$grade)){ echo "checked=\"checked\""; }} else { if (empty($_SESSION['error4']) and preg_match("/^2$/",$_SESSION['grade'])){ echo "checked=\"checke    d\""; }} ?> /><label for="grade-2">2</label>
