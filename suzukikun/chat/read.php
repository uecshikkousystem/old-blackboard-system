...
if (isset($_GET['mode'])) {
	if (preg_match("/^list(get|conti)$/",$_GET['mode'])) {
		$sql = "select * from " . $tablename_chat . " where (addr = '" . $pos_tmp . "' or addr = 'all' or position = '" . $pos_tmp . "') ORDER BY id DESC";	
		...
		$sql = "select id from " . $tablename_chat . " where ((id > '" . $_COOKIE['newid'] . "') and (addr = '" . $pos_tmp . "' or addr = 'all' or position = '" . $pos_tmp . "'))";
		$res = mysql_query($sql, $conn);
		$num = mysql_num_rows($res);
